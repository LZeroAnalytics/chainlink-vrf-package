...
func deployContracts(e helpers.Environment) {
	fs := flag.NewFlagSet("ocr2vrf-deploy-contracts", flag.ExitOnError)
	...
	weiPerUnitLink := fs.String("wei-per-unit-link", "6e16", "wei per unit link price for feed")
	beaconPeriodBlocks := fs.Int64("beacon-period-blocks", 3, "beacon period in blocks")
	maxCallbackGasLimit := fs.Uint("max-cb-gas-limit", 2500000, "max callback gas limit")
	maxCallbackArgumentsLength := fs.Uint("max-cb-args-length", 32*10, "max callback arguments length")
	...
	if *linkEthFeed == "" {
		fmt.Println("Deploying LINK-ETH feed...")
		feedAddress = helpers.DeployLinkEthFeed(e, link.String(), decimal.RequireFromString(*weiPerUnitLink).BigInt())
	} else {
		feedAddress = common.HexToAddress(*linkEthFeed)
	}
	...
	// Create JSON output with all addresses
	type Addresses struct {
		DKG            string `json:"dkg"`
		VRFCoordinator string `json:"vrf_coordinator"`
		VRFBeacon      string `json:"vrf_beacon"`
		LinkToken      string `json:"link_token,omitempty"`
		LinkEthFeed    string `json:"link_eth_feed,omitempty"`
	}

	addresses := Addresses{
		DKG:            dkgAddress.String(),
		VRFCoordinator: vrfCoordinatorAddress.String(),
		VRFBeacon:      vrfBeaconAddress.String(),
	}

	// Only include LINK token if we deployed it
	if *linkAddress == "" {
		addresses.LinkToken = link.String()
	}

	// Only include LINK-ETH feed if we deployed it
	if *linkEthFeed == "" {
		addresses.LinkEthFeed = feedAddress.String()
	}

	// Marshal to JSON and print
	jsonOutput, err := json.MarshalIndent(addresses, "", "  ")
	if err != nil {
		fmt.Println("Error generating JSON:", err)
		os.Exit(1)
	}

	fmt.Println("CONTRACT_ADDRESSES_JSON_BEGIN")
	fmt.Println(string(jsonOutput))
	fmt.Println("CONTRACT_ADDRESSES_JSON_END")
}

func testConsumerFlow(e helpers.Environment) {
	fs := flag.NewFlagSet("ocr2vrf-test-consumer-flow", flag.ExitOnError)

	vrfCoordinatorAddressStr := fs.String("vrf-coordinator-address", "", "VRF coordinator address")
	beaconPeriodBlocks := fs.Int64("beacon-period-blocks", 3, "beacon period in blocks")
	subscriptionBalanceString := fs.String("subscription-balance", "1e19", "amount to fund subscription")
	linkAddressStr := fs.String("link-address", "", "LINK token address")
	numWords := fs.Uint("num-words", 1, "number of words to request")
	confDelay := fs.Int64("conf-delay", 1, "confirmation delay")

	helpers.ParseArgs(fs, os.Args[2:], "vrf-coordinator-address", "link-address")

	vrfCoordinatorAddress := common.HexToAddress(*vrfCoordinatorAddressStr)
	link := common.HexToAddress(*linkAddressStr)

	fmt.Println("Deploying beacon consumer...")
	consumerAddress := deployVRFBeaconCoordinatorConsumer(e, vrfCoordinatorAddress.String(), false, big.NewInt(*beaconPeriodBlocks))
	fmt.Println("Beacon consumer deployed at:", consumerAddress.String())
	...
	fmt.Println("Requesting Randomness...")
	requestID := requestRandomnessFromConsumer(e, consumerAddress.String(), uint16(*numWords), subID, big.NewInt(*confDelay))
	time.Sleep(120 * time.Second) // Adjust this based on your block time
	fmt.Println("Redeeming Randomness...")
	redeemRandomnessFromConsumer(e, consumerAddress.String(), subID, requestID, int64(*numWords))
}
